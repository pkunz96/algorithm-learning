...
def sample(size: int) -> Tuple[NDArray, NDArray, Dict[int, str]]:
    x_data, y_data, local_label_name_dict, = generate_training_data_record(heap_sort, gen_heap_sort_environment, lambda x: "T_{0}".format(x), size)
...
matplotlib.use('Agg')
grid_search = GridSearch("optimizer_experiment_heap_sort_sgd");
