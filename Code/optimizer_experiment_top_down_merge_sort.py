...
def sample(size: int) -> Tuple[NDArray, NDArray, Dict[int, str]]:
    x_data, y_data, local_label_name_dict, = generate_training_data_record(top_down_merge_sort, gen_top_down_merge_sort_environment, lambda x: "T_{0}".format(x), size)
...
matplotlib.use('Agg')
grid_search = GridSearch("optimizer_experiment_top_down_merge_sort_sgd");
...
grid_search.search(sample, name_for_label, lambda x: SGD(learning_rate = x), custom_early_stopping, True)
