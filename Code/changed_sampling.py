...
# Tested
def generate_problem(mu: int, sigma: int) -> List[int]:
    problem_size: int = np.random.normal(problem_size_mu, problem_size_sigma, 1)
    return [ceil(value) for value in np.random.normal(mu, sigma, max([ceil(problem_size), 2]))]


# Tested
def sample(sort: Callable[[Environment], List[int]], create_environment: Callable[[List[int]], Environment], mu: int, sigma: int) -> NDArray[int]:
    env: Environment = create_environment(generate_problem(mu, sigma))
    sort(env)
...
def generate_training_data_record(sort: Callable[[Environment], List[int]], create_environment: Callable[[List[int]], Environment], name_label: Callable[[int], str], sample_size: int, mu: int, sigma: int) -> Tuple[NDArray, NDArray, Dict[int, str]]:
    raw_predictor_arr: NDArray[int] = np.array(sample(sort, create_environment, mu, sigma))
    ...
    while not completed:
        raw_predictor_arr = np.append(raw_predictor_arr, sample(sort, create_environment, mu, sigma), axis=0)
        computation_state_successor_dict = generate_state_successor_dict(raw_predictor_arr)
        completed = determine_record_size(raw_predictor_arr, computation_state_successor_dict) >= sample_size
